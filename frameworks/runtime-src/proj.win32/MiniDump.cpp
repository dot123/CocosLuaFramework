...
{
	time_t timeExcep;
	WCHAR szTime[64] = {0}, szPath[MAX_PATH] = {0};
	WCHAR *pTmp;


	// 创建dump文件夹
	::GetModuleFileName(NULL, szPath, MAX_PATH * sizeof(WCHAR));
	pTmp = _tcsrchr(szPath, '\\');	
	if (pTmp != NULL)
		*pTmp = '\0';
	strDump = pTmp;
	strDump += L"Dump\\";
	...
	// 获取时间，组成dump文件名
	timeExcep = time(NULL);
	_tcsftime(szTime, sizeof(szTime), L"%Y%m%d%H%M%S", localtime(&timeExcep));
	strDump += L"DumpFile_";
	strDump += szTime;
	strDump += L".dmp";
